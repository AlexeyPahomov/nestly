function isActiveCommitment(status) {
    return status !== 'COMPLETED' && status !== 'CANCELLED';
}
/** Суммы по активным обязательствам (без COMPLETED / CANCELLED). */
export function sumPlannedExpenseCommitments(rows) {
    let planned = 0;
    let reserved = 0;
    for (const row of rows) {
        ...
        const reservedPortion = Math.min(Math.max(0, row.reserved_amount), row.amount);
